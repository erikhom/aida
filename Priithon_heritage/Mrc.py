...
class Mrc:
    def __init__(self, path, mode='r', extHdrSize=0, extHdrNints=0, extHdrNfloats=0):
        ...
        self.data_offset = 1024 + self.hdr.__next__
...
class Mrc2:
    ...
    def _initFromExistingFile(self):
        ...
        self._extHdrSize        = self.hdr.__next__
    ...
    def makeExtendedHdr(self, numInts, numFloats, nSecs=None):
        ...
        self._extHdrSize        = self.hdr.__next__ = minExtHdrSize(nSecs,self._extHdrBytesPerSec)
        self._dataOffset   = self._hdrSize + self._extHdrSize
...
def hdrInfo(hdr):
    ...
    print("# extended header size:       ", hdr.__next__, end=' ')
    if hdr.__next__ > 0:
        n = numInts + numFloats
        if n>0:
            print(" (%d secs)" % (hdr.__next__/(4. * n) ,))
        else:
            print(" (??? secs)")
