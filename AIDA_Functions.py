...
def ProcessSetImageFilename():   
    ...
    if type(Set.PSF_filenames) in (list, tuple):
        ...
        for i in range(len(Set.image_filenames)):
            ...
            if len(temp_list) == 0:
                ...
                if Set.image_format == 'f':
                    ...
                    temp = N.asarray(Image.open('%s%s' %(Set.image_directory[i], 
        ...
        if Set.image_format == 'f':
            ...
            temp = N.asarray(Image.open('%s%s' %(Set.image_directory[-1], 
...
def ProcessSetPSFFilename():   
    ...
    if type(Set.PSF_filenames) in (list, tuple):
        ...
        if Set.PSF_format == 'f':
            ...
            PSFfilelist = AGF.PickFiles(directory=Set.PSF_directory,
                    prefix=Set.PSFfilebase, suffix='.tiff') #@ AGF
...
def SetupShapeParamsTempArrays():   
    ...
    Set.realFFTshape = Set.shape[:-1] + (round(Set.shape[-1]/2 + 1),)
    ...
    if Set.origin_centered_PSF_flag:
        ...
        if Set.dataset_label:
            ...
            return '%s%s_cleanPSF_o_%s_%s' %( Set.results_directory, 
                    Set.dataset_label, Set.timehms, str.split(filebase, 
                    sep='.')[0])
        else:

            return '%scleanPSF_o_%s_%s' %( Set.results_directory, Set.timehms,
                    str.split(filebase, sep='.')[0])     
    else:
...
def ProcessImageBackgroundNoiseSettings():   
    ...
    ## Set.background
    if type(Set.background) in (list, tuple):
    ...
    if type(Set.dark_image) in (list, tuple):
    ...
    ## Set.sigma_det
    ## for Nframes decon or Nimages/file
    if type(Set.sigma_det) in (list, tuple):
...
def StoreImageData():   
    ...
    if Set.images_in_file == 'multiple':
        ...
        filecheck = str.split(Set.image_list[0], sep='.')
...
def Start_Print():   
    ...
    if Set.info_level >= 1:
        ...
        if type(Set.image_directory) in (list, tuple) and \
                len(Set.image_directory) > 1:
        
            print('Image file base =', end=' ')
        ...
        if type(Set.PSF_directory) in (list, tuple) and \
                len(Set.PSF_directory) > 1:

            print('PSF file base =', end=' ')
        ...
        print('-   -   -   -   -   -   -   -   -')
        print('Initial object guess =', Set.initial_object_guess, end=' ')
...
#           raise ValueError, message
        ...
        print('Maximum uphill object moves allowed before breaking =', end=' ') 
        print(Set.max_uphill_object_PCG_steps)
        print('Maximum uphill PSF moves allowed before breaking =', end=' ') 
        print(Set.max_uphill_PSF_PCG_steps)
        print('Rising tolerance ratio between estimate steps =', end=' ')
        ...
        print('Number of sequential AIDA stop alerts before breaking =', end=' ')
        print(Set.max_sequential_PCG_stops)
        print('Maximum PCG stop signals before skipping optimization =', end=' ')
        print(Set.max_optimization_stops) 
        print('Maximum uphill signals before skipping optimization =', end=' ')
        ...
        if len(Set.lambda_object_exp_grid) > 1:
            ...
            print('lambda_object multipier =', end=' ')
        ...
        if Set.lambda_object_input is None:
            ...
            print("'lambda_object' will be calculated as: ", end=' ')
    ...
    if Set.info_level >= 2:
        ...
        print('\tgradient operator = ', Set.derivative_operator, end=' ')
        print('\tlaplacian operator = ', Set.laplacian_operator, end=' ')
        ...
        print('\tgradient norm type =', Set.norm, end=' ')
        ...
        print('\tsubtract PSF background percent =', end=' ')
        print(Set.PSF_subtract_background_percent)
        print('\tclean nsigmas to subtract =', Set.nsigmas)
        print('\tPSF percent threshold set to fill value =', end=' ') 
        print(Set.PSF_threshold_percent)
        print('\tOTF percent threshold set to fill value =', end=' ') 
        ...
        print('\tradially averaged OTF constraint weight (dimensions):', end=' ')
    ...
    print("=======")
    print()
    print("Deconvolution Start Time:", end=' ')
    ...
    for i in range(len(Set.image_list)):
        ...
        if Set.memory_usage_level > 0:
            ...
            if Set.info_level >= 2:
                ...
                if Set.initial_object_guess == 'wiener':
                    ...
                    print('wiener = %.6g' %Set.wiener, end=' ') 
        ...
        for j in range(len(Set.lambda_object_exp_grid)):
            ...
            if len(Set.lambda_object_exp_grid) > 1:
                ...
                print("... lambda_object grid_search exponent =", end=' ') 
                print(Set.lambda_object_exp, "of", end=' ') 
            ...
            for k in range(len(Set.theta_exp_grid)):
                ...
                if len(Set.theta_exp_grid) > 1:
                    ...
                    print("... theta grid_search exponent =", end=' ') 
                    print(Set.theta_object_exp, "of", end=' ') 
    ...
    if Set.info_level >= 1:
        ...
        try:
            ...
            print('lambda_object: %.6g (min), %.6g (max), %.6g (mean), %.6g (std)' \
                    %U.mmms(Set.lambda_object), end=' ')
        except:
        
            print('lambda_object: %.6g' %Set.lambda_object, end=' ')
...
def Convergence_Print(check_object, seq_object_stops, seq_object_risings,
    ...
    if Set.decon_type == 'classical':
        ...
        if Set.info_level >= 2:
            ...
            print('\t\t>>> obj::', check_object, '\tseq_stops:', end=' ') 
            print(seq_object_stops, '  seq_rising:', seq_object_risings, end=' ') 
            print('  stops:', object_stops, '<<<')
    elif Set.terms[0] == 0:

        if Set.info_level >= 2:

            print('\t\t>>> PSF::', check_PSF, '\tseq_stops:', seq_PSF_stops, end=' ') 
            print('  seq_rising:', seq_PSF_risings, end=' ') 
...
def PCG_Decon_Print(results_index, fn, optimization_round, start_decon_time):
    ...
    if Set.info_level >= 1:
        ...
        try:
            ...
            print('\n<lambda_object>: %.6g' %Set.lambda_object.mean(), end=' ')
        except:
        
            print('\nlambda_object: %.6g' %Set.lambda_object, end=' ')
...
#
        try:
            ...
            print('   <lambda_OTF>: %.6g' %Set.lambda_OTF.mean(), end=' ')
        except:
        
            print('   lambda_OTF: %.6g' %Set.lambda_OTF, end=' ')
        ...
        print()
        print('| total number of while-loop decon iterations: ', end=' ') 
        print(optimization_round - 1)
        print('| total number of CG calls/iterations: ', end=' ')
        print(Set.decon_total_CG_itns, '(obj PSF tot)')
        print('| total number of Cost Function calculations: ', end=' ')
        print(Set.decon_total_CostFunction_itns, '(obj PSF tot)')
        print('| total time of deconvolution: ', end=' ') 
...
def GenOutputFilenames(file):
        ...
        if file == '' and Set.images_in_file == 'single':
            ...
            if Set.dataset_label:
                ...
                objresultsfile = '%sRobj_%s_%s_%s_L%s' %(
                        ...
                        str.split(Set.image_list, sep='.')[0], lambdastr)
            else:
            
                objresultsfile = '%sRobj_%s_%s_L%s' %(Set.results_directory,
                        Set.timehms, str.split(Set.image_list, sep='.')[0], 
                        lambdastr)
                ...
                if Set.origin_centered_PSF_flag:
                    ...
                    if Set.dataset_label:
                        ...
                        PSFresultsfile = '%sRpsf_o_%s_%s_%s' %(
                                ...
                                Set.timehms, str.split(Set.image_list, 
                                sep='.')[0])
                    else:

                        PSFresultsfile = '%sRpsf_o_%s_%s' %(
                                Set.results_directory, Set.timehms,
                                str.split(Set.image_list, sep='.')[0])
                else:
            ...
            if Set.dataset_label:
                ...
                objresultsfile = '%sRobj_%s_%s_%s_L%s' %(
                        Set.results_directory, Set.dataset_label, Set.timehms,
                        str.split(Set.image_list[0], sep='.')[0], lambdastr)
            else:
            
                objresultsfile = '%sRobj_%s_%s_L%s' %(Set.results_directory,
                        Set.timehms, str.split(Set.image_list[0], 
                        sep='.')[0], lambdastr)
            ...
            if Set.decon_type != 'classical':
                ...
                if Set.origin_centered_PSF_flag:
                    ...
                    if Set.dataset_label:
                        ...
                        PSFresultsfile = '%sRpsf_o_%s_%s' %(
                                Set.results_directory, Set.timehms,
                                str.split(Set.image_list[0], sep='.')[0])
                else:
            ...
            if Set.dataset_label:
                ...
                objresultsfile = '%sRobj_%s_%s_%s_L%s' %(
                        Set.results_directory, Set.dataset_label, Set.timehms,
                        str.split(Set.image_list[file], sep='.')[0], 
                        lambdastr)
            else:
            
                objresultsfile = '%sRobj_%s_%s_L%s' %(Set.results_directory,
                        Set.timehms, str.split(Set.image_list[file],
                        sep='.')[0], lambdastr)
    ...
    ###  nobjects Decon Output  ###
            ...
            for i in range(len(Set.image_list)):
                ...
                if Set.dataset_label:
                    ...
                    objresultsfile.append('%sRobj_%s_%s_%s_L%s_%s' %(
                            Set.results_directory, Set.dataset_label, 
                            Set.timehms, str.split(Set.image_list[i], 
                            sep='.')[0], AGF.Num2Str(
                    ...
                    objresultsfile.append('%sRobj_%s_%s_L%s_%s' %(
                            Set.results_directory, Set.timehms,
                            str.split(Set.image_list[i], sep='.')[0],
        ...
        # PSF
        if type(Set.imagefilebase) in (list, tuple) and \
                len(Set.imagefilebase) > 1:
        
            temp = str.split(Set.imagefilebase[0], sep='.')[0] + '---' + \
                    str.split(Set.imagefilebase[-1], sep='.')[0]
        else:
                ...
                for i in range(len(Set.image_list)):
                    ...
                    if Set.dataset_label:
                        ...
                        PSFresultsfile.append('%sRpsf_o_%s_%s_%s' %(
                                Set.results_directory, Set.timehms,
                                str.split(Set.image_list[i], sep='.')[0], i))
            else:
...
def File_Decon_Print(hypersearch_total_CG_itns, 
    ...
    if Set.decon_type == 'classical':
        ...
        if Set.info_level >= 1 and len(Set.lambda_object_exp_grid) > 1 and \
            ...
            print('*** *** *** *** *** *** ***')
            print('|| TOTAL CG ITERATIONS FOR HYPERPARAMETER SEARCH:', end=' ')
            print(hypersearch_total_CG_itns[0], '(object only)')
            print('|| TOTAL COST FUNCTION ITERATIONS FOR HYPERPARAMETER', end=' ')
            print('SEARCH:', end=' ')
            print(hypersearch_total_CostFunction_itns[0], '(object only)')
            print('|| TOTAL TIME FOR HYPERPARAMETER SEARCH: ', end=' ')
...
def Final_Print():
    ...
    if Set.info_level >= 1:
        ...
        if Set.decon_type == 'classical':
            ...
            print('Total Cost Function calculations =', end=' ') 
            print(Set.cum_CostFunction_itns[0])
            print('Average CG iteration time: (object only)', end=' ')
            AGF.PrintTime(Set.cum_CG_time[0], Set.cum_CG_itns[0]) #@ AGF
            print()
            print('Average CostFunction calculation time: (object only)', end=' ')
            ...
            print('Average CG iteration time: (PSF only)', end=' ')
            AGF.PrintTime(Set.cum_CG_time[1], Set.cum_CG_itns[1]) #@ AGF
            print()
            print('Average CostFunction calculation time: (PSF only)', end=' ')
