...
def Check_Settings(Set):   #Fun
    ...
    if Set.dataset_list is not None:
        ...
        if type(Set.dataset_list) in (list, tuple):
    ...
    if Set.dataset_list is None:
        ...
        if Set.image_filenames == '':
            ...
            Set.image_filenames = input("Image file to deconvolve: ")
        ...
        if type(Set.image_filenames) is str:
        ...
        elif type(Set.image_filenames) in (list, tuple):

            # assume a simple list
            if type(Set.image_filenames[0]) is str:
    ...
    if Set.dataset_list is None:
        ...
        if Set.PSF_filenames == '':
            ...
            Set.PSF_filenames = input("PSF(s) to use: ")
        ...
        if type(Set.PSF_filenames) is str:
        ...
        elif type(Set.PSF_filenames) in (list, tuple):
            ...
            elif type(Set.PSF_filenames[0]) is str:
            ...
            elif type(Set.PSF_filenames[0]) in (list, tuple):
    ...
    if Set.dataset_list is None:
        ...
        if type(Set.dataset_label) in (str, type(None)):
            ...
            Set.dataset_label_list = len(Set.dataset_image_prefix_list) * \
                    [Set.dataset_label]
        elif type(Set.dataset_label) in (list, tuple):
            ...
            elif type(Set.dataset_label[0]) in (str,
                    type(None)):
    ...
    ### if opt == '-r'
    if type(Set.results_directory) is str:
    
        Set.results_directory_list = len(Set.dataset_image_prefix_list) * \
                [Set.results_directory]
    elif type(Set.results_directory) in (list, tuple):
    ...
    ### if opt == '-b'
    if type(Set.background) in (int, int, float, 
            type(None)): # or isinstance(Set.background,  N.number):
    
        Set.dataset_background_list = len(Set.dataset_image_prefix_list) * \
                [Set.background]
    elif type(Set.background) in (list, tuple):
        ...
        if len(Set.background) == 1:
            ...
            for i in range(len(Set.background)):
                ...
                if type(Set.background[i]) in (list, tuple):
    ...
    ### if opt == '-s'  
    if type(Set.sigma_det) in (int, int, float, 
            type(None)):
        
            Set.dataset_sigma_det_list = len(Set.dataset_image_prefix_list) * \
                    [Set.sigma_det]
    elif type(Set.sigma_det) in (list, tuple):
...
#                   [Set.sigma_det[0]]
        if len(Set.sigma_det) == len(Set.dataset_image_prefix_list) or \
            ...
            for i in range(len(Set.sigma_det)):
                ...
                elif type(Set.sigma_det[i]) in (list, tuple):
    ...
    if Set.dark_image == '' or Set.dark_image is None:
    ...
    elif type(Set.dark_image) is str:
        ...
        if os.path.isfile(Set.dark_image):
            ...
            message = "\n'dark_image' file ('-a' option) specified does " + \
                    "not exist!"
            raise ValueError(message)
    elif type(Set.dark_image) in (list, tuple):
        ...
        if len(Set.dark_image) == 1 and Set.dark_image[0] in ('', None):
            ...
            for i in range(len(Set.dark_image)):
                ...
                elif type(Set.dark_image[i]) is str:
    ...
    ### if opt == '-l'
    if type(Set.lambda_object_scaling) not in (int, int, 
            float):
    ...
    ### if opt == '-t'
    if type(Set.theta_scaling) not in (int, int, 
            float):
    ...
    ### if opt == '-v'
    if type(Set.lambda_OTF_scaling) not in (int, int, 
            float):
    ...
    ### if opt == '-u'
    if type(Set.lambda_PSF_scaling) not in (int, int, 
            float):
    ...
    ### if opt == '--terms'
    if type(Set.terms) is not tuple:
    ...
    ### if opt == '--clean'
    if type(Set.cleaning) not in (list, tuple) or \
            len(Set.cleaning) != 4:
    ...
    ### if opt == '--backper'
    if type(Set.PSF_subtract_background_percent) not in (int, 
            float):
    ...
    ### if opt == '--nsig'
    if type(Set.nsigmas) not in (int, float):
    ...
    ### if opt == '--psfthresh'
    if type(Set.PSF_threshold_percent) not in (int, float):
    ...
    ### if opt == '--otfthresh'
    if type(Set.OTF_threshold_percent) not in (int, float):
    ...
    ### if opt == '--fill'
    if type(Set.fill) not in (int, float):
    ...
    ### if opt == '--ufloor'
    if type(Set.u_floor_per_dim) not in (int, float):
    ...
    ### if opt == '--vfloor'
    if type(Set.v_floor_per_dim) not in (int, float):
    ...
    ### if opt == '--objtol'
    if type(Set.object_PCG_tolerance) not in (int, float):
    ...
    ### if opt == '--psftol'
    if type(Set.PSF_PCG_tolerance) not in (int, float):
    ...
    ### if opt == '--pcgiterarr'
    if type(Set.PCG_iter_array) not in (list, tuple):
    ...
    elif type(Set.object_PCG_iter_array) not in (list, 
            tuple):
    ...
    if type(Set.PSF_PCG_iter_array) not in (list, tuple):
    ...
    ### if opt == '--maxiter'
    if type(Set.max_total_PCG_blocks) is not int:
    ...
    ### if opt == '--maxseqstops'
    if type(Set.max_sequential_PCG_stops) is not int:
    ...
    ### if opt == '--maxupobj'
    if type(Set.max_uphill_object_PCG_steps) is not int:
    ...
    ### if opt == '--maxuppsf'
    if type(Set.max_uphill_PSF_PCG_steps) is not int:
    ...
    ### if opt == '--risetol'
    if type(Set.rising_tol_ratio) is not float:
    ...
    ### if opt == '--fracconverge'
    if type(Set.Nframes_fractional_convergence) is not float:
    ...
    ### if opt == '--objccgtol'
    if type(Set.object_CCG_tolerance) is not float:
    ...
    ### if opt == '--psfccgtol'
    if type(Set.PSF_CCG_tolerance) is not float:
    ...
    ### if opt == '--objccgiter'
    if type(Set.max_object_CCG_iter) is not int:
    ...
    ### if opt == '--psfccgiter'
    if type(Set.max_PSF_CCG_iter) is not int:
    ...
    ### if opt == '--ccgxmin'
    if type(Set.xmin_value) is not float:
    ...
    ### if opt == '--ccgivec'
    if type(Set.ivec_value) is not int:
