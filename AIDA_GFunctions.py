...
def PrintTime(timeinseconds, denominator=1.): # used in 'AIDA_Functions.py'
    ...
    if timeinseconds <= 60:
        ...
        print('%.4g' %timeinseconds, 'secs', end=' ')
    elif 60 < timeinseconds <= 3600:

        print('%.4g' %(timeinseconds/60.), 'mins', end=' ')
    else:

        print('%.4g' %(timeinseconds/3600.), 'hrs', end=' ') 
####
...
def WienerWeight(dark_image, object):   
    ...
    shape = object.shape[:-1] + (round(object.shape[-1]/2) + 1,)
...
def CentroidOverSquare(array, center=None, size=None, background_threshold=0):
    ...
    lower = list(map(int, center-size/2.+1))
    upper = list(map(int, center+size/2.+1))
...
def ConditionalResizePSF(PSF, cropshape, center, condition, fill=0.):
    ...
    if (condition == '1' and np.product(PSF.shape) > np.product(cropshape)) or \
        ...
        if type(center) is not bytes:    # i.e. 'origin' or
                                                    # 'array_center'
